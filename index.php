...
// Иначе, если запрос был методом POST, т.е. нужно проверить данные и сохранить их в XML-файл.
  ...
  if (!empty($_COOKIE[session_name()]) &&
    ...
    try {
    $stmt=$db->prepare("UPDATE application1 SET id = ?, name = ?, email = ?, birth_date = ?, sex = ?, amount_of_limbs = ?, biography = ? where id=$uid"); 
    $stmt -> execute([$uid, $_POST['name'], $_POST['email'], $_POST['birth_date'], $_POST['sex'], $_POST['amount_of_limbs'], $_POST['biography']]);
    }
